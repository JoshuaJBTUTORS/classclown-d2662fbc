...
export const CleoInteractiveLearning: React.FC<CleoInteractiveLearningProps> = ({
  ...
  const recentMessages = useRef<Array<{
    role: 'user' | 'assistant';
    content: string;
  }>>([]);
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('cleo_messages').select('*').eq('conversation_id', currentConversationId).order('created_at', {
        ascending: true
      });
      if (error) {
    ...
    const channel = supabase.channel(`messages:${currentConversationId}`).on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'cleo_messages',
  ...
  useEffect(() => {
    ...
    const isGoodbyeMessage = (content: string) => {
      ...
      return lowerContent.includes('bye') || lowerContent.includes('goodbye') || lowerContent.includes('see you') || lowerContent.includes('take care') || lowerContent === 'you too!' || lowerContent === 'thanks!';
    };
  ...
  const handleModeSwitch = async (newMode: ChatMode, isAuto: boolean = false) => {
    ...
    if (conversationId) {
      await supabase.from('cleo_conversations').update({
    ...
    const transitionMessage: CleoMessage = {
      ...
      conversation_id: conversationId || '',
      role: 'assistant',
      content: isAuto ? newMode === 'text' ? "We've switched to text mode since we've used up our voice time. Let's continue learning!" : "Let's switch back to voice for this next part!" : newMode === 'text' ? "Great! Let's practice with some questions. Type your answers below." : "Perfect! Let me explain this next part with voice.",
      mode: newMode,
      created_at: new Date().toISOString()
    };
    ...
    toast({
      ...
      description: newMode === 'voice' ? 'Voice conversation activated' : 'Type your messages below'
    });
  };
  ...
  const handleBackClick = async () => {
    ...
    if (connectionState === 'connected') {
      const confirmed = window.confirm("You're currently in a lesson. Going back will pause your progress. Continue?");
      if (!confirmed) return;
  ...
  return <div className="min-h-screen bg-white">
      ...
      <div className="flex items-center justify-between px-8 pt-6 pb-4 max-w-[1120px] mx-auto">
        <div className="flex items-center gap-4">
          <button onClick={handleBackClick} className="cleo-back-btn">
        ...
        <div className="flex items-center gap-3">
          {/* Progress Indicator */}
          <LessonProgressIndicator totalSteps={lessonData.steps.length} completedSteps={completedSteps} activeStep={activeStep} />
          
          {/* Show real-time usage during voice session */}
          {connectionState === 'connected' && <MinuteUsageTracker sessionStartTime={new Date(sessionStartTime)} />}
      ...
      <div className="cleo-interactive-wrapper">
        <div className="cleo-layout">
          ...
          <section className="cleo-lesson-card">
            ...
            {lessonPlan && lessonPlan.learning_objectives.length > 0 && <div className="cleo-lesson-body">
            ...
            <div className="mt-6">
              ...
              const answerMessage = correct ? `The student answered correctly: "${answerText}". Acknowledge this briefly and continue teaching.` : `The student answered incorrectly: "${answerText}". Provide gentle correction and explanation.`;
              if (connectionState === 'connected') {
            ...
            }} conversationId={conversationId || null} onContentAction={(contentId, action, message) => {
          ...
          <aside className="cleo-panel">
            <div>
              <CleoAvatar isSpeaking={isSpeaking} isListening={isListening} isMuted={isMuted} size="large" />
            ...
            {allMessages.length > 0 && allMessages[allMessages.length - 1].role === 'assistant' && <div className="cleo-panel-text">
                {allMessages[allMessages.length - 1].content}
              </div>}

            {/* Progress Card */}
            {lessonPlan && lessonPlan.teaching_sequence && <div className="cleo-progress-card-new">
                <div className="cleo-progress-title">Lesson Progress</div>
                {lessonPlan.teaching_sequence.map((step, index) => <div key={step.id || index} className="cleo-progress-item">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden max-h-64">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={handlePauseLesson} disabled={connectionState !== 'connected' && mode !== 'text'} className="flex-1">
                <Pause className="w-3 h-3 mr-1" />
                Pause
              </Button>

              {allStepsCompleted && <div className="flex flex-col gap-2 w-full mt-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  {connectionState === 'disconnected' && <Button onClick={() => {
                ...
                handleVoiceConnect();
                  ...
                  <Button onClick={handleCompleteLesson} className="w-full bg-green-600 hover:bg-green-700 text-white" size="sm">
      ...
      {/* Practice Dialog */}
      {conversationId && <AssignPracticeDialog isOpen={showPracticeDialog} onClose={() => setShowPracticeDialog(false)} incorrectTopics={questionStats?.incorrect_answers > 0 ? ['Review incorrect answers'] : []} conversationId={conversationId} />}
    </div>;
};