    ...
    return (
      <>
        <Card className="flex flex-row overflow-hidden hover:shadow-lg transition-all duration-200 border-gray-200 bg-white">
          <div className="w-48 h-32 bg-gradient-to-br from-primary/10 to-primary/20 relative flex-shrink-0">
            ...
            {course.status === 'published' && course.price && (
              <Badge className="absolute bottom-2 left-2 bg-primary text-white text-xs px-2 py-1">
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <Badge className="bg-primary/10 text-primary text-xs">{course.subject || "General"}</Badge>
                    <Badge 
                      variant="outline" 
      ...
      <Card className="overflow-hidden flex flex-col h-full hover:shadow-lg transition-all duration-200 border-gray-200 bg-white group">
        <div className="h-48 bg-gradient-to-br from-primary/10 to-primary/20 relative overflow-hidden">
          {course.cover_image_url ? (
            ...
            <div className="flex items-center justify-center h-full">
              <BookOpen className="h-12 w-12 text-primary group-hover:scale-110 transition-transform duration-200" />
            </div>
          )}
          ...
          {course.status === 'published' && course.price && (
            <Badge className="absolute bottom-3 left-3 bg-primary text-white shadow-sm backdrop-blur-sm">
              {formatPrice(course.price)}/month
            </Badge>
          )}
        </div>
