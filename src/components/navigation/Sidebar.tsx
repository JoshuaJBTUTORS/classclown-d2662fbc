...
const Sidebar: React.FC<SidebarProps> = ({ isOpen = true }) => {
  ...
  const sidebarWidth = isCoursePage ? 'w-48' : 'w-64';
  ...
  return (
    <div className={`flex flex-col h-full bg-background border-r ${sidebarWidth}`}>
      <div className="flex items-center justify-center p-4 border-b border-gray-100">
        <img 
          src="/lovable-uploads/d35d104e-dca8-466e-8820-20dcc5131ad3.png" 
          alt="Class Clown Logo" 
          className={`${isCoursePage ? 'h-20' : 'h-30'} w-auto`}
        />
      </div>
          ...
          {filteredNavItems.map((item) => (
            <li key={item.name}>
              ...
              >
                ...
                <span className={`font-medium ${isCoursePage ? 'text-sm' : ''}`}>{item.name}</span>
              </NavLink>
            </li>
          ))}
        </ul>
