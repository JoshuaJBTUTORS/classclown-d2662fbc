...
const statusClass: Record<GoalStatus, string> = {
  'achieved': 'text-violet-300 border-violet-400/30 bg-violet-400/10',
  'on-track': 'text-sky-300 border-sky-400/30 bg-sky-400/10',
  'behind': 'text-amber-300 border-amber-400/30 bg-amber-400/10',
  'not-achieved': 'text-rose-300 border-rose-400/30 bg-rose-400/10',
};

const barClass: Record<GoalStatus, string> = {
  'achieved': 'bg-violet-400',
  'on-track': 'bg-sky-400',
  'behind': 'bg-amber-400',
  'not-achieved': 'bg-rose-400',
};
...
const Tile: React.FC<TileProps> = ({ icon, label, value, sub, loading, onClick }) => (
  ...
  >
    <div className="flex items-center gap-2 text-[#8e8ea0] text-xs mb-2">
      {icon}
      <span>{label}</span>
    </div>
    {loading ? (
    ...
    <div className="text-xs text-[#8e8ea0] mt-2">{loading ? ' ' : sub}</div>
  </button>
);
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-baseline justify-between gap-2 mb-1.5">
        <span className="text-xs text-[#8e8ea0] truncate">{label}</span>
      ...
      <div className="text-sm font-medium mb-1.5">
        {loading ? '—' : fmt(current)}
        <span className="text-[#8e8ea0] font-normal"> / {target.toLocaleString()}</span>
      </div>
  ...
  return (
    <div className="w-full max-w-2xl mb-8">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs uppercase tracking-wide text-[#8e8ea0]">Today · {today}</span>
        <button
          onClick={refresh}
          className="p-1.5 rounded-lg hover:bg-white/10 text-[#8e8ea0] transition-colors"
      ...
      <div className="mt-3 p-4 rounded-2xl border border-white/10">
        <button
          onClick={() => navigate('/goals')}
          className="flex items-center gap-1.5 text-xs uppercase tracking-wide text-[#8e8ea0] hover:text-white transition-colors mb-4"
        >
