...
BEGIN
  ...
  LOOP
    ...
    -- Use the most recent actual session in the series as the template so that
    -- renames, subject changes and roster changes carry forward.
    SELECT * INTO template_record
    FROM public.lessons
    WHERE parent_lesson_id = lesson_record.id
      AND start_time < now()
      AND EXISTS (SELECT 1 FROM public.lesson_students ls WHERE ls.lesson_id = lessons.id)
    ORDER BY start_time DESC
    LIMIT 1;

    IF template_record.id IS NULL THEN
      template_record := lesson_record;
    END IF;
    ...
    WHERE ls.lesson_id = template_id;
    ...
    WHILE working_date <= extension_date LOOP
      ...
      IF NOT EXISTS (
        ...
        INSERT INTO public.lessons (
          ...
          new_start_ts,
          new_end_ts,
          template_record.is_group,
          'scheduled',
          template_record.subject,
          template_record.lesson_type,
          false,
          true,
          working_date,
          NULL, NULL, NULL,
