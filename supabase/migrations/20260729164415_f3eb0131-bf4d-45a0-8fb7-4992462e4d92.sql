...
BEGIN
  ...
  IF normalized !~* '^\(*\s*(select|with|table|values|explain)\b' THEN
  ...
  IF normalized ~* '^\s*explain\b' THEN
    IF normalized ~* '\banalyze\b' OR normalized ~* '\banalyse\b' THEN
