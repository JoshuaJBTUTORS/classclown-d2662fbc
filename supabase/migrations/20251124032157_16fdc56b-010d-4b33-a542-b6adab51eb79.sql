-- ADD Bonus Cleo Minutes (24/11/2025 - 30/11/2025 8pm)
-- Corrected: Only update bonus_minutes and total_minutes_allowed to prevent double-counting

WITH lesson_counts_per_student AS (
  -- Count lessons per student (in the target date range)
  SELECT 
    s.id as student_id,
...
accounts_to_credit AS (
  -- Determine which accounts to credit
...
SET 
  ...
  -- NOTE: NOT updating minutes_remaining to prevent double-counting in UI
