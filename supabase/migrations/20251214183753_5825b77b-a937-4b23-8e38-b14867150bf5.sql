-- Auto-assign Winter Term Exams to students based on lessons from Dec 15-22, 2025
WITH lesson_students_data AS (
  ...
  WHERE l.start_time >= '2025-12-15'::timestamp
    AND l.start_time < '2025-12-23'::timestamp
...
subject_to_assessment AS (
  SELECT * FROM (VALUES
    -- GCSE/Year 11 Maths (including typo variants)
    ('GCSE Maths Higher', '0dd7de16-d0d8-422d-adbe-4945fab2af2f'::uuid),
    ('GCSE Maths Highier', '0dd7de16-d0d8-422d-adbe-4945fab2af2f'::uuid),
    ...
    ('Year 11 Maths Highier', '0dd7de16-d0d8-422d-adbe-4945fab2af2f'::uuid),
    ...
    -- GCSE Chemistry
    ('GCSE Chemistry', (SELECT id FROM ai_assessments WHERE title = 'GCSE Chemistry Paper 1 - Winter Term Exam' LIMIT 1)),
    ...
    -- Early KS2
    ('Early KS2 English', (SELECT id FROM ai_assessments WHERE title = 'Early KS2 - Winter Term Exam' LIMIT 1)),
    ('Early KS2 Maths', (SELECT id FROM ai_assessments WHERE title = 'Early KS2 - Winter Term Exam' LIMIT 1)),
    -- 11 Plus
    ...
    ('11 Plus NVR', (SELECT id FROM ai_assessments WHERE title = '11 PLUS NVR - Winter Term Exam' LIMIT 1))
...
assignments_to_create AS (
  SELECT DISTINCT
    ...
    '2025-12-22 23:59:59'::timestamp with time zone as due_date
  FROM lesson_students_data lsd
  JOIN subject_to_assessment sta ON LOWER(lsd.subject) = LOWER(sta.subject_name)
  WHERE sta.assessment_id IS NOT NULL
)
