...
BEGIN
  ...
  IF trimmed !~* '^\(*\s*(select|with|table|values|explain)\b' THEN
  ...
  IF trimmed ~* '^\s*explain\b' THEN
    IF trimmed ~* '\banalyze\b' OR trimmed ~* '\banalyse\b' THEN
      RAISE EXCEPTION 'agent_cleo_exec rejected EXPLAIN ANALYZE (got: %)', preview;
    END IF;
    -- Strip leading EXPLAIN and optional (...) options block for downstream keyword checks.
    explain_body := regexp_replace(trimmed, '^\s*explain\s*(\([^)]*\))?\s*', '', 'i');
    IF explain_body !~* '^\(*\s*(select|with|table|values)\b' THEN
  ...
  IF trimmed ~* '(^|[^a-z_])(insert|update|delete|merge|alter|create|drop|truncate|grant|revoke|copy|call|do|execute|perform|set|reset|listen|notify|unlisten|vacuum|reindex|cluster|refresh|discard|lock|comment|security|prepare|deallocate|commit|rollback|savepoint|release)([^a-z_]|$)' THEN
