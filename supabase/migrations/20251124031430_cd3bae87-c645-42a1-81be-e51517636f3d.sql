-- ROLLBACK: Remove bonus Cleo minutes (24/11/2025 - 30/11/2025 8pm)
-- Using the same corrected aggregation logic but subtracting
...
accounts_to_credit AS (
  -- Now determine which accounts to debit
  -- Student accounts (if they exist)
  SELECT 
    lc.student_user_id as target_user_id,
    SUM(lc.lesson_count * 10) as minutes_to_remove
...
SET 
  bonus_minutes = bonus_minutes - atc.minutes_to_remove,
  minutes_remaining = minutes_remaining - atc.minutes_to_remove,
  total_minutes_allowed = total_minutes_allowed - atc.minutes_to_remove
