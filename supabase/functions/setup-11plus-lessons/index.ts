...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!roleData || !['admin', 'owner'].includes(roleData.role)) {
      throw new Error('Only admins and owners can setup lesson plans');
    }

    console.log('Starting 11 Plus lesson plans setup...');

    // Define all lesson plans
    const lessonPlans = [
    ...
    console.log(`Successfully inserted ${lessonPlans.length} lesson plans`);

    return new Response(
      JSON.stringify({ 
        success: true, 
        message: `Successfully created ${lessonPlans.length} 11 Plus lesson plans`,
        subjects: ['11 Plus Maths', '11 Plus English', '11 Plus Verbal Reasoning', '11 Plus Non-Verbal Reasoning'],
        weeks_per_subject: 30
      }),
      {
