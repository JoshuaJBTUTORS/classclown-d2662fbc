...
import { StudentInviteEmail } from "./_templates/student-invite-email.tsx";
...
serve(async (req) => {
  ...
  try {
    ...
    const { email, firstName, lastName, studentId } = await req.json() as InviteRequest;
    ...
    const { data: notification, error: notificationError } = await supabase
      ...
      .insert({
        ...
        type: 'student_invite',
        email: email,
        subject: 'Student invitation sent',
        status: 'pending'
      })
      .select()
      .single();
    ...
    const html = await renderAsync(
      React.createElement(StudentInviteEmail, {
        firstName,
        lastName,
        inviteLink,
    ...
    console.log(`Student invitation sent successfully to ${email}`);
